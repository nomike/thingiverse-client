# Changelog

All notable changes to this project will be documented in this file. This format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.1.1](https://github.com/nomike/thingiverse-client/compare/v1.1.0...v1.1.1) (2026-03-15)


### Bug Fixes

* treat tags and ancestors as primitive arrays in PATCH thing body ([#15](https://github.com/nomike/thingiverse-client/issues/15)) ([9e0d863](https://github.com/nomike/thingiverse-client/commit/9e0d863e068fa3e1f3a44c894ab9b79c73d24a98))

## [1.1.0](https://github.com/nomike/thingiverse-client/compare/v1.0.0...v1.1.0) (2026-03-14)


### Features

* add PyPI publish workflow on release ([#13](https://github.com/nomike/thingiverse-client/issues/13)) ([738446e](https://github.com/nomike/thingiverse-client/commit/738446e38800d842a836ba803bad428bbc2358f7))

## [1.0.0](https://github.com/nomike/thingiverse-client/compare/v0.2.2...v1.0.0) (2026-03-14)


### ⚠ BREAKING CHANGES

* The Python package has been renamed from `thingiverse_client` to `thingiverse`. Update imports from `from thingiverse_client import ...` and `thingiverse_client.api` / `thingiverse_client.models` to `from thingiverse import ...` and `thingiverse.api` / `thingiverse.models`.

### Code Refactoring

* rename Python package from thingiverse_client to thingiverse ([#11](https://github.com/nomike/thingiverse-client/issues/11)) ([2c592ef](https://github.com/nomike/thingiverse-client/commit/2c592ef5862dd45846b7b7f9ecb8f7ee46494ca1))

## [0.2.2](https://github.com/nomike/thingiverse-client/compare/v0.2.1...v0.2.2) (2026-02-22)


### Bug Fixes

* **docs:** classify HTTP-verb-prefixed models and add missing mkdocs-material dep ([#8](https://github.com/nomike/thingiverse-client/issues/8)) ([6a3e38f](https://github.com/nomike/thingiverse-client/commit/6a3e38f264ce657fcf0212e8d0802049618adaab))

## [0.2.1](https://github.com/nomike/thingiverse-client/compare/v0.2.0...v0.2.1) (2026-02-22)


### Documentation

* auto-generate full API reference with mkdocs-gen-files ([#6](https://github.com/nomike/thingiverse-client/issues/6)) ([e4ca1b5](https://github.com/nomike/thingiverse-client/commit/e4ca1b56a4de31d2d94e5a95bc01f3008a4c2368))

## [0.2.0](https://github.com/nomike/thingiverse-client/compare/v0.1.0...v0.2.0) (2026-02-22)


### Features

* regenerate full SDK from complete Thingiverse OpenAPI spec ([#4](https://github.com/nomike/thingiverse-client/issues/4)) ([6c0b1ff](https://github.com/nomike/thingiverse-client/commit/6c0b1ffb45e5b7aa16aa307db4d95bd9acabbc87))

## 0.1.0 (2026-02-22)


### Features

* initial Thingiverse Python SDK ([02da182](https://github.com/nomike/thingiverse-client/commit/02da1821bb4a45a01ad8962c32978ff35e9ca965))

## [0.1.0] - Initial release

- Python SDK generated from Thingiverse OpenAPI spec
- Production and staging base URL constants (`BASE_URL_PRODUCTION`, `BASE_URL_STAGING`)
- `Client` and `AuthenticatedClient` with configurable `base_url`
- Unit tests (mocked) and optional integration/destructive tests
- Pre-commit (commitlint, check-mkdocs, Ruff, markdown, yaml)
- CI: tests, pre-commit, release-please, docs to GitHub Pages
